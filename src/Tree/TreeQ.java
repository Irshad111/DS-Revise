...
public class  TreeQ {
	...
	// 4. lCA in bst lowest common ancestor in binary search tree
	...
	void printleft(Node node){ // in this we are not printing leave node
	    if(node==null){
	        return;
	    }
	    if(node.left!=null){
...
static int res;
	 int NumberOfTurns(Node root, int first, int second)
	{
		...
		res=0;
		count(lca.left,first,second,0,res);
		count(lca.right,first,second,1,res);
	...
	static void count(Node node,int first,int second,int dir,int turns){
		if(node==null){
			return;
		}
		...
		if(dir==0){//we are in left side of node
			count(node.left,first,second,0,turns);
			count(node.right,first,second,1,turns+1);
		}else{ // dir=1 right
			count(node.left,first,second,0,turns+1);
			count(node.right,first,second,1,turns);
		}
	}
}
